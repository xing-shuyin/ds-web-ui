/**
 * Lightweight bash fallback for Windows (ported from pi-web-ui ensure-bash.ts).
 *
 * When neither Git Bash nor a bash on PATH exists, download busybox-w32
 * (single self-contained exe, no installer) into <home>/.ds-web/bin/ and
 * expose it as bash.exe — busybox dispatches on argv[0], so `bash.exe` runs
 * its bash (ash) applet. The terminal panel (terminals.js) and, via PATH,
 * any bash-using tool then both resolve to it, so the terminal never silently
...
/** Directory holding the busybox fallback (shared with terminals.js / PATH). */
export function windowsBashDir() {
	return join(homedir(), ".ds-web", "bin");
}

/** bash.exe (busybox bash applet) used by the terminal and PATH consumers. */
export function windowsBashPath() {
	return join(windowsBashDir(), "bash.exe");
}

/** True when a standard Git Bash install exists (preferred over busybox). */
